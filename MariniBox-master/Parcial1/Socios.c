...
void modf_Socios(eSocio lista[], int indice)
{
    ...
    do
    {
        ...
        case 3:
            ...
            fflush(stdin);
            getString("Ingrese telefono (debe tener un guion): ",auxNewTelefono);
        ...
        case 5:
            ...
            fflush(stdin);
            getString("ingrese email(datos alfanumericos): ",auxNewEmail);
...
int baja_Socios(eSocio lista[], int tam)
{
    ...
    for(i=0; i<tam; i++)
    {
        if(lista[i].isEmpty==OCUPADO)
        {
            printf("\nLegajo empleado %d",lista[i].codigoSocio);
        }
    }
